...
D3D12_SHADER_BYTECODE CShader::CompileShaderFromFile(tstring pszFileName, LPCSTR pszShaderName, LPCSTR pszShaderProfile, ID3DBlob **ppd3dShaderBlob)
{
...
#endif
	WCHAR* msg = nullptr;
	// WCHAR* msg{ pszFileName.c_str() };
	D3DCompileFromFile(msg, NULL, NULL, pszShaderName, pszShaderProfile, nCompileFlags, 0, ppd3dShaderBlob, NULL);
...
void CShader::CreateShaderVariables(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	m_pd3dcbGameObject = Radar::Util::CreateBuffer(NULL, ncbElementBytes, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, NULL);

	// 업로드 타입으로 버퍼를 만들어 최종적으로 상수 버퍼가 된다.

	m_pd3dcbGameObject->Map(0, NULL, (void **)&m_pcbMappedGameObject);
	// 이를 맵을 해서 포인터를 얻어 쉐이더가 가지게 된다.
}
...
{
	return(CompileShaderFromFile(_XTT("Shaders.hlsl"), "VSDiffused", "vs_5_1", ppd3dShaderBlob));
}

D3D12_SHADER_BYTECODE CDiffusedShader::CreatePixelShader(ID3DBlob **ppd3dShaderBlob)
{
	return(CompileShaderFromFile(_XTT("Shaders.hlsl"), "PSDiffused", "ps_5_1", ppd3dShaderBlob));
}
