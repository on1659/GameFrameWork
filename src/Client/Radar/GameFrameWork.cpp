...
WARP_RESULT_ENUM CGameFrameWork::Start(void* pData)
{
	...
	cout << "[Start] CmdList Close" << endl;
	gGraphicsCommandList->Close();
	ID3D12CommandList *ppd3dCommandLists[] = { gGraphicsCommandList };
	cout << "[Start] ExecuteCommandLists" << endl;
	gCommandQueue->ExecuteCommandLists(1, ppd3dCommandLists);

	cout << "[Start] WaitForGpu - fence=" << m_pd3dFence.Get() << " event=" << m_hFenceEvent << " idx=" << m_nSwapChainBufferIndex << endl;
	WaitForGpuComplete();
	cout << "[Start] WaitForGpu Done" << endl;
...
void CGameFrameWork::Render()
{
	...
	if (bFirstFrame)
	{
		cout << "[Render] === First Frame Diagnostic ===" << endl;
		cout << "[Render] gDevice=" << gDevice << endl;
		cout << "[Render] gCommandQueue=" << gCommandQueue << endl;
		cout << "[Render] SwapChain=" << m_pDXGISwapChain << endl;
		cout << "[Render] BackBuffer[0]=" << m_ppd3dSwapChainBackBuffers[0].Get() << endl;
		cout << "[Render] BackBuffer[1]=" << m_ppd3dSwapChainBackBuffers[1].Get() << endl;
		cout << "[Render] BufferIndex=" << m_nSwapChainBufferIndex << endl;
		cout << "[Render] ClientSize=" << m_nWndClientWidth << "x" << m_nWndClientHeight << endl;
		if (gDevice)
		{
			HRESULT removedReason = gDevice->GetDeviceRemovedReason();
			cout << "[Render] DeviceRemovedReason=0x" << std::hex << removedReason << std::dec << endl;
		}
		bFirstFrame = false;
	}

	if (!gDevice || !gCommandQueue || !m_pDXGISwapChain)
	{
		cout << "[Render] SKIP: device/queue/swapchain null" << endl;
		return;
	}

	if (!m_ppd3dSwapChainBackBuffers[0])
	{
		cout << "[Render] BackBuffers not created yet, skipping" << endl;
		return;
	}

	HRESULT hResult = gCommandALlocator->Reset();
	if (FAILED(hResult))
		cout << "[Render] Allocator Reset FAILED hr=0x" << std::hex << hResult << std::dec << endl;
	hResult = gGraphicsCommandList->Reset(gCommandALlocator, NULL);
	if (FAILED(hResult))
		cout << "[Render] CmdList Reset FAILED hr=0x" << std::hex << hResult << std::dec << endl;
	...
	if (FAILED(hResult))
		cout << "[Render] CmdList Close FAILED hr=0x" << std::hex << hResult << std::dec << endl;
	...
	if (FAILED(hResult))
	{
		static bool bPresentErrorOnce = false;
		if (!bPresentErrorOnce)
		{
			bPresentErrorOnce = true;
			HRESULT reason = gDevice->GetDeviceRemovedReason();
			cout << "[Render] Present FAILED hr=0x" << std::hex << hResult << endl;
			cout << "[Render] DeviceRemovedReason=0x" << reason << std::dec << endl;
		}
	}
...
WARP_RESULT_ENUM CGameFrameWork::CreateDeivce()
{
	...
	if (FAILED(hResult) || pDevice == nullptr)
	{
	...
	// Fence
	cout << "[Device] pDevice=" << pDevice << endl;
	ID3D12Fence* pFenceRaw = nullptr;
	hResult = pDevice->CreateFence(0, D3D12_FENCE_FLAG_NONE, __uuidof(ID3D12Fence), (void **)&pFenceRaw);
	cout << "[Device] CreateFence HR=0x" << std::hex << hResult << std::dec << " fence=" << pFenceRaw << endl;
	m_pd3dFence = pFenceRaw;
...
WARP_RESULT_ENUM CGameFrameWork::CreateSwapChain()
{
	...
	HRESULT hResult = m_pdxgiFactory->CreateSwapChain(gCommandQueue, &dxgiSwapChainDesc, &pSwapChain);
	if (pSwapChain)
	{
		pSwapChain->QueryInterface(__uuidof(IDXGISwapChain3), (void**)&m_pDXGISwapChain);
		pSwapChain->Release();
	}
#endif
...
WARP_RESULT_ENUM CGameFrameWork::OnResizeBackBuffers(const WORD width, const WORD height)
{
	...
	// DX12 초기화 전에 WM_SIZE가 올 수 있으므로 체크
	if (!gDevice || !gCommandQueue || !m_pDXGISwapChain)
	{
		cout << "[OnResize] Skipped - DX12 not ready" << endl;
		return WARP_RESULT_ENUM::OK;
	}
	cout << "[OnResize] Running w=" << width << " h=" << height << endl;

	cout << "[OnResize] 1-WaitForGpu" << endl;
	WaitForGpuComplete();
	cout << "[OnResize] 2-ResetCmdList" << endl;
	gGraphicsCommandList->Reset(gCommandALlocator, nullptr);

	cout << "[OnResize] 3-ResetBackBuffers" << endl;
	for (int i = 0; i < m_nSwapChainBuffers; i++)
		m_ppd3dSwapChainBackBuffers[i].Reset();

	m_pd3dDepthStencilBuffer.Reset();

	cout << "[OnResize] 4-ResizeBuffers" << endl;
#ifdef _WITH_ONLY_RESIZE_BACKBUFFERS
	DXGI_SWAP_CHAIN_DESC dxgiSwapChainDesc;
	...
	DXGI_SWAP_CHAIN_DESC dxgiSwapChainDesc;
	m_pDXGISwapChain->GetDesc(&dxgiSwapChainDesc);
	HRESULT resizeHr = m_pDXGISwapChain->ResizeBuffers(m_nSwapChainBuffers, m_nWndClientWidth, m_nWndClientHeight, dxgiSwapChainDesc.BufferDesc.Format, dxgiSwapChainDesc.Flags);
	cout << "[OnResize] ResizeBuffers HR=0x" << std::hex << resizeHr << std::dec << endl;
	if (FAILED(resizeHr))
	{
		HRESULT reason = gDevice->GetDeviceRemovedReason();
		cout << "[OnResize] DeviceRemovedReason=0x" << std::hex << reason << std::dec << endl;
		return WARP_RESULT_ENUM::FAIL;
	}
	m_nSwapChainBufferIndex = 0;
#endif

	cout << "[OnResize] 5-CreateRTV" << endl;
	CreateRenderTargetViews();
	cout << "[OnResize] 6-CreateDSV" << endl;
	CreateDepthStencilView();

	cout << "[OnResize] 7-CloseCmdList" << endl;
	gGraphicsCommandList->Close();

	ID3D12CommandList *ppd3dCommandLists[] = { gGraphicsCommandList };
	gCommandQueue->ExecuteCommandLists(1, ppd3dCommandLists);

	cout << "[OnResize] 8-WaitForGpu" << endl;
	WaitForGpuComplete();
	cout << "[OnResize] Done" << endl;
...
#pragma region [Creaet Signature]
	...
	int index = 0;
	pd3dRootParameters[index].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
	...
	pd3dRootParameters[index].Constants.Num32BitValues = 32;
	...
	{
		if (pd3dErrorBlob)
			MessageBoxA(NULL, (char*)pd3dErrorBlob->GetBufferPointer(), "RootSig Serialize Error", MB_OK);
		return;
	}
...
#pragma region [Create Mesh & GameObject & Shader]
	...
	*/
	...
	// int boxCBufIndex = 0;//�����ε���!
	// cbAddress += boxCBufIndex * elementByteSize;
	// 
	// D3D12_CONSTANT_BUFFER_VIEW_DESC cbvDesc;
	// cbvDesc.BufferLocation = cbAddress;      //�ܽ���Ʈ ������ �ּ�
	...
	if (vsShader == nullptr)
	{
		MessageBoxA(NULL, "vsShader make fail - check working directory", "create fail", MB_OK);
		return;
	}
	...
	*/
	...
	gDevice->CreateShaderResourceView(m_pTexture.Get(), &srvDesc, hDescriptor);//��������......
	// Creaet Resourec 
	...
	// Rasterizer State - explicit init
	...
	if (hr != S_OK)
	{
		...
		std::string fullMsg = "pipeState make fail\nHRESULT: 0x";
		char hexBuf[16];
		sprintf_s(hexBuf, "%08X", (unsigned int)hr);
		fullMsg += hexBuf;
		fullMsg += "\n\nRootSig: ";
		fullMsg += (m_mRootSignature.Get() ? "OK" : "NULL");
		fullMsg += "\nVS: ";
		fullMsg += (vsShader ? "OK" : "NULL");
		fullMsg += "\nGS: ";
		fullMsg += (gsShader ? "OK" : "NULL");
		fullMsg += "\nPS: ";
		fullMsg += (psShader ? "OK" : "NULL");
		if (!details.empty())
		{
			fullMsg += "\n\n--- D3D12 Debug ---\n";
			fullMsg += details;
		}
		MessageBoxA(NULL, fullMsg.c_str(), "create fail", MB_OK);
	}
...
{
	if (!m_mRootSignature.Get() || !pipeState.Get())
	{
		cout << "[RenderCube] SKIP: RootSig=" << m_mRootSignature.Get() << " PSO=" << pipeState.Get() << endl;
		return;
	}
	...
	pd3dCommandList->SetGraphicsRootDescriptorTable(2, m_pd3dCbvSrvDescriptorHeap.Get()->GetGPUDescriptorHandleForHeapStart());//�ؽ��� �����ϱ�.
	pd3dCommandList->SetGraphicsRootDescriptorTable(3, m_SamplerHeap->GetGPUDescriptorHandleForHeapStart()); //���÷������ϱ�
